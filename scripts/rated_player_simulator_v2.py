...
class RatedPlayersSimulator(GameSimulator):
    ...
    def play_from_scrimmage(self):
        ...
        if play_score < TURNOVER_BASE_PROB + TURNOVER_WEIGHT * off_riskiness_rating:
            ...
            # case of sack / tfl
            print("Sack!")
            yards_gained = - np.random.poisson(max(5 + defensive_explosiveness - offensive_consistency, 0))
            ...
            print("normal play")
            yards_gained = np.random.poisson(max(5.3 + offensive_consistency - defensive_consistency, 0))
